...
3. **Inputs** — All `daily-logs/<role>/*.md` since last successful run (tracked in `scripts/.compile_state.json`), every existing `knowledge/concepts/*.md`, `knowledge/connections/*.md`, every `knowledge/agents/<role>/*.md`, and `knowledge/index.md`. Reads the lint gate prompt from `scripts/prompts/filing_gate.md`.
