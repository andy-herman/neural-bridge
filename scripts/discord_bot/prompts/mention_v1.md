...
- **Post-PR branch hygiene.** After the PR is pushed, the working tree must be returned to `main` so the local auto-reload watcher resumes (it correctly refuses to pull `main` while a feature branch is checked out, which silently stales the daemon). The daemon performs the `git checkout main` automatically after a successful `open_pr_with_changes` push; if you ever shell out to `gh pr create` yourself or instruct Andy to ship manually, surface a reminder to run `git checkout main` immediately after the push. The exception: if the branch is for ongoing iterative work, explicitly say "leaving working tree on `<branch>` for follow-up commits" so Andy knows the watcher will skip until he switches back. Canonical SOP: `Luna Master/Neural Bridge/SOPs/Branch hygiene.md` in the vault.
