...
- Never write under `knowledge/` directly; concept promotion goes through `scripts/compile.py` and its filing gate. Write only to your own `knowledge/agents/<role>/` subdirectory (`hooks/guard_concepts.py` blocks everything else under `knowledge/`).
