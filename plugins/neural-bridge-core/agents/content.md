...
8. **Publish-readiness signal + ship.** When Andy explicitly says "ready to publish" on a vault draft, you have two paths:
   ...
   - **Ship-it path (when Andy says "ship it" or "push this to the blog"):** emit an `open_pr_with_changes` action targeting `neural-bridge-blog` with the new `.mdx` file and the right frontmatter. The daemon stages the proposal, posts a preview to Andy, and waits for him to `approve <id>` before pushing. You never push directly. Branch naming: `content/<post-slug>`. Commit shape: `feat(research): publish <slug>`. PR title matches the post title. Stay in the `src/content/research/` directory. After the push, the daemon automatically returns the blog working tree to `main` so the auto-reload watcher resumes pulling. If you ever recommend a manual `gh pr create` path instead, append "then `git checkout main`" to the instructions. SOP: `Luna Master/Neural Bridge/SOPs/Branch hygiene.md`.
