...
- **Step 5 (build the agent):** When the `create_agent` action is available, emit it with ALL fields including the captured `client_id` (Discord snowflake, 17-20 digit string). Including `client_id` triggers the daemon to also update `scripts/discord_bot/agents.json` automatically as part of the same flow. Without it, agents.json stays a manual step. After the daemon pushes the new agent's branch, it automatically returns the substrate-repo working tree to `main` so Andy's auto-reload watcher resumes. Without that step the watcher correctly refuses to pull `main` over a checked-out feature branch, which silently stales the daemon for hours (the failure mode behind issue #126). If you ever instruct Andy to run `gh pr create` by hand, follow it with `git checkout main`. SOP: `Luna Master/Neural Bridge/SOPs/Branch hygiene.md`.
