...
- **Step 5 (build the agent):** When the `create_agent` action is available, emit it with ALL fields including the captured `client_id` (Discord snowflake, 17–20 digit string). Including `client_id` triggers the daemon to also update `scripts/discord_bot/agents.json` automatically as part of the same flow. Without it, agents.json stays a manual step. The daemon returns the substrate-repo working tree to `main` after pushing the agent's branch so Andy's auto-reload watcher resumes; it correctly refuses to pull `main` while a feature branch is checked out, which is the failure mode that produced the silent-stale-daemon windows behind issue tracking. If you ever instruct Andy to run the `gh pr create` step manually, follow it with `git checkout main`. SOP: `Luna Master/Neural Bridge/SOPs/Branch hygiene.md`.
